...
pub fn view<'a>(state: &'a AppState) -> Option<Element<'a, Message>> {
    ...
    // --- Identity section ---
    let vcs = status.map(|s| s.identity.vcs_kind.to_string()).unwrap_or_else(|| "—".into());
    let path = project.path.clone();
    let remote = status.and_then(|s| s.remote.upstream.clone()).unwrap_or_else(|| "—".into());
    ...
    let status_col = if let Some(s) = status {
        ...
        let conflict = if s.conflict.has_conflict { "Yes" }
            else if s.conflict.detection_unavailable { "Unknown" }
            else { "No" };
    ...
    let recent_ops: Vec<Element<'_, Message>> = state.operation_logs.iter()
        .rev()
        .filter(|log| log.result.per_project.iter().any(|pp| &pp.project_id == id))
        .take(5)
        .map(|log| {
            let ok = log.result.per_project.iter()
                .find(|pp| &pp.project_id == id)
                .map(|pp| pp.success)
                .unwrap_or(false);
            let icon = if ok { "✓" } else { "✗" };
            text(format!("{} {} — {}", icon, log.result.kind, log.result.started_at.format("%m/%d %H:%M").to_string()))
                .size(11)
                .into()
        })
        .collect();

    let recent = column(std::iter::once(text("Recent operations").size(12).into())
    ...
    let refresh_btn = button(text("Refresh").size(12))
        .on_press(Message::Project(ProjectMessage::StatusRefreshRequested(id.clone())));

    let fetch_btn = button(text("Fetch").size(12))
        .on_press(Message::Project(ProjectMessage::FetchRequested(id.clone())));

    let remove_btn = button(text("Remove from workspace").size(12))
        .on_press(Message::Workspace(WorkspaceMessage::RemoveProjectRequested(id.clone())));
