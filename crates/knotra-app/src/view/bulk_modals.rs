...
use crate::{
    message::{
        ChangelogMessage, ConflictOpsMessage, ContextMessage, FreezerMessage,
        Message, SyncMessage,
    },
    state::AppState,
};
...
pub fn pull_modal(state: &AppState) -> Element<'_, Message> {
    ...
    let project_rows: Vec<Element<'_, Message>> = match &sync.phase {
        crate::state::sync::SyncPhase::Idle => {
            let selected: Vec<_> = state.workspace.as_ref()
                .map(|ws| ws.projects.iter()
                    .filter(|p| state.selection.contains(&p.id))
                    .collect())
                .unwrap_or_default();
            selected.iter().map(|p| {
                text(format!("• {}", p.name)).size(12).into()
            }).collect()
        }
        ...
        crate::state::sync::SyncPhase::FetchRunning { done, total } => {
            vec![text(format!("Fetching… {}/{}", done, total)).size(12).into()]
        }
        crate::state::sync::SyncPhase::PullRunning { completed, .. } => {
            completed.iter().map(|p| {
                let name = project_name_for(state, &p.project_id);
                text(format!("⟳ {}", name)).size(12).into()
            }).collect()
        }
        crate::state::sync::SyncPhase::Done(result) => {
            result.per_project.iter().map(|pp| {
    ...
    let project_list = scrollable(
        column(project_rows).spacing(4)
    ).height(Length::Fixed(200.0));

    let footer: Element<'_, Message> = match &sync.phase {
        crate::state::sync::SyncPhase::Idle => {
            button(text("Plan Pull").size(13))
        ...
        crate::state::sync::SyncPhase::AwaitingConfirm(_) => {
            row![
                button(text("Execute").size(13))
                    .on_press(Message::Sync(SyncMessage::ExecuteRequested)),
        ...
        crate::state::sync::SyncPhase::Planning => {
            text("Pulling…").size(12).into()
        }
        crate::state::sync::SyncPhase::Done(_) => {
...
pub fn tag_modal(state: &AppState) -> Element<'_, Message> {
    ...
    let validation: Element<'_, Message> = match &freezer.phase {
        ...
        crate::state::freezer::FreezerPhase::ValidationReady(validation) => {
            let blockers: Vec<Element<'_, Message>> = validation.entries.iter()
                .filter(|v| v.is_blocked())
                .map(|v| text(format!("✗ {}: {}", v.project_name,
    ...
    let footer: Element<'_, Message> = match &freezer.phase {
        crate::state::freezer::FreezerPhase::Idle => {
        ...
        crate::state::freezer::FreezerPhase::Executing => {
            text("Tagging…").size(12).into()
        }
        crate::state::freezer::FreezerPhase::Done(_) => {
            button(text("Close").size(13))
                .on_press(Message::Freezer(FreezerMessage::BulkModalClosed))
                .into()
        }
...
pub fn switch_modal(state: &AppState) -> Element<'_, Message> {
    ...
    let footer: Element<'_, Message> = match ctx.phase {
        crate::state::context::ContextPhase::Idle => {
        ...
        crate::state::context::ContextPhase::Done(_) => {
            button(text("Close").size(13))
...
pub fn resolve_panel<'a>(state: &'a AppState, project_id: &'a ProjectId) -> Element<'a, Message> {
    ...
    let file_rows: Vec<Element<'_, Message>> = ops.cached.values().flat_map(|d| d.conflicted_files.iter()).map(|f| {
...
pub fn changelog_modal(state: &AppState) -> Element<'_, Message> {
    ...
    let content: Element<'_, Message> = match &cl.phase {
        crate::state::changelog::ChangelogPhase::Idle => {
        ...
        crate::state::changelog::ChangelogPhase::Ready(draft) => {
            column![
                scrollable(text(format!("{:?}", draft)).size(11))
