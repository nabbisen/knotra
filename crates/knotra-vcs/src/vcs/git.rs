    ...
    match AsyncRepository::open(path).await {
        ...
        Ok(repo) => {
            ...
            let context = repo.status_digest().await.ok().map(|d| VcsContext {
                ...
                branch: if d.current_branch.starts_with("(detached") {
                    None
                } else {
...
pub(crate) fn gix_ahead_behind(repo_path: &str) -> RemoteStatus {
    let inner = || -> Option<RemoteStatus> {
        ...
        let repo = endringer_git::GitBackend::open(std::path::Path::new(repo_path)).ok()?;
...
pub async fn list_tags_blocking(path: &str) -> Vec<String> {
    ...
    tokio::task::spawn_blocking(move || {
        let repo = endringer_git::GitBackend::open(&p).ok()?;
        use endringer_core::backend::VcsBackend;
        repo.list_tags_sorted(SortOrder::NewestFirst)
            .ok()
...
pub(crate) async fn tag_create_annotated(
    ...
    .unwrap_or_else(|e| ProjectOperationResult {
        ...
        stderr: String::new(),
        exit_code: None,
        error_message: Some(format!("task join: {e}")),
    })
}
...
pub async fn tag_exists(project: &Project, tag_name: &str) -> bool {
    ...
    tokio::task::spawn_blocking(move || {
        use endringer_core::backend::VcsBackend;
        endringer_git::GitBackend::open(&path)
            .ok()
    ...
    if stash_applied {
        ...
        if !pop.success {
            let hint = RecoveryHint {
                ...
                suggested_commands: vec![format!("cd {:?} && git stash pop", project.path)],
...
pub async fn switch_context(
    ...
    let uncommitted = wt_status
        .as_ref()
        .map(|s| s.staged.len() + s.unstaged.len())
        .unwrap_or(0);
    let untracked = wt_status.as_ref().map(|s| s.untracked.len()).unwrap_or(0);
    ...
    if uncommitted > 0 {
        let hint = RecoveryHint {
            ...
            suggested_commands: vec![format!(
                "cd {:?} && git stash push -m before-switch",
                project.path
            )],
            see_also: None,
        };
...
pub async fn validate_for_freeze(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let backend = match endringer_git::GitBackend::open(&path) {
            Ok(b) => b,
            Err(e) => {
                return FreezeValidationEntry {
                    project_id: project.id.clone(),
                    project_name: project.name.clone(),
                    included,
                    is_clean: false,
        ...
        if uncommitted > 0 {
            blockers.push(format!("{uncommitted} uncommitted change(s)"));
        }
        if untracked > 0 {
            notes.push(format!("{untracked} untracked file(s)"));
        }
        if tag_exists {
            blockers.push(format!("tag '{}' already exists", name));
        }
